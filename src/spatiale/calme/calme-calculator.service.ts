...
import { TypeElemBruitService } from '../type_elem_bruit/type_elem_bruit.service';
...
export class CalmeCalculatorService {
  ...
  constructor(
    private readonly typeElemBruitService: TypeElemBruitService,
  ) {}
  ...
  async loadTypeElemBruitConfig(): Promise<void> {
    try {
      ...
      const types = await this.typeElemBruitService.findAll();
      
      this.configCache.clear();
      for (const type of types) {
        this.configCache.set(type.idTypeElemBruit, {
          poids: type.poids,
          dHalf: type.dHalf,
          type: type.typeElemBruit
        });
      console.log(`=>Type ${type.idTypeElemBruit} (${type.typeElemBruit}): poids=${type.poids}, d_half=${type.dHalf}m`);
